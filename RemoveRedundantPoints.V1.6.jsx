...
function removeRedundantPoints(path, redundantPointSets, keepLeadingPoint, keepTrailingPoint, keepAveragedPoint){
        ...
        for (i=0; i<redundantPointSets.length; i++) {
                ...
                if (keepLeadingPoint && keepTrailingPoint) {
                         ...
                         * If we are to keep the trailing point, copy its position to the leading point.
                         * If we are to keep neither leading or trailing point, average the position
                         * of all the redundant points.
                         */
                        if (keepTrailingPoint) {
                                leadingPoint.anchor = trailingPoint.anchor;
                        }
                        else if (!keepLeadingPoint && !keepTrailingPoint) {
                                for (j=0; j<currentPointSet.length; j++) {
                                        x += path.pathPoints[currentPointSet[j]].anchor[0];
                                        y += path.pathPoints[currentPointSet[j]].anchor[1];
                                }
                        ...
                        /*
                         * Copy the rightDirection from the trainingPoint to the leadingPoint
                         */
                        leadingPoint.rightDirection = trailingPoint.rightDirection;
                        /*
                         * Handle the PointType in a minimal manner.  If only the rightHandle is extended, copy
                         * the trailingPoint PointType to the leadingPoint.  If only the leftHandle is extended,
                         * keep the leadingPoint PointType unchanged.  If both handles are extended, pick the point
                         * type based on the angles of the two direction handles.  Calculate the distance from the
                         * anchor to each the leftDirection and rightDirection.  If both handles have the same angle
                         * relative to the X axis, make the PointType SMOOTH, otherwise make it CORNER.
                         */
                        var leftDistance = Math.round(getPairDistance(leadingPoint.anchor, leadingPoint.leftDirection));
                        var rightDistance = Math.round(getPairDistance(leadingPoint.anchor, leadingPoint.rightDirection));
                        if (leftDistance > 0 && rightDistance > 0) {
                                var leftTheta = getPairTheta(leadingPoint.anchor, leadingPoint.leftDirection);
                                var rightTheta = getPairTheta(leadingPoint.anchor, leadingPoint.rightDirection);
...
function doInitDialog(dlgInit) {
        var defaultTolerance = 2.0;
        var maxSliderTolerance = 2;
        ...
        dlgInit.selectionPnl.ignoreSelected.value = true;
        ...
        (dlgInit.tolerancePnl.editText = dlgInit.tolerancePnl.add('edittext', undefined, defaultTolerance)).helpTip="Enter a tolerance value.  Values greater then 2.0 or more precise than 1/100 point can be manually entered here.";
        ...
        (dlgInit.removalPnl.doKeepLeadingPoint = dlgInit.removalPnl.add('checkbox', undefined, 'Keep Leading Point' )).helpTip = "Keep the leading point (lowest path index).";
        (dlgInit.removalPnl.doKeepTrailingPoint = dlgInit.removalPnl.add('checkbox', undefined, 'Keep Trailing Point' )).helpTip = "Keep the trailing point (highest path index).";
        ...
        dlgInit.removalPnl.doKeepLeadingPoint.value = true;
...
function initActionOk() {
        ...
        if (dlgInit.tolerancePnl.editText.text > 2.0) {
                proceed = confirm("Tolerance entered greater than 2.0 PostScript points.  Proceed?");
        }
        if (proceed) {
                dlgInit.hide();
        }
